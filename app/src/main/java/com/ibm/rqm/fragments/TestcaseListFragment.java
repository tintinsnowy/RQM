...
import com.ibm.rqm.adapter.TestcaseAdapter;
import com.raizlabs.android.dbflow.sql.language.Select;

/**
 * A fragment representing a list of Items.
 */
public class TestcaseListFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener{
    TextView mHeader;
    ListView mList;
    ...
    private void setUpList(){
        if (mAdapter == null){
            mAdapter = new TestcaseAdapter(getActivity().getBaseContext());
        }
        mList.setAdapter(mAdapter);
        long workItemNum = new Select().count().from(Testcase.class).count();
        mHeader.setText("There are " + workItemNum + " testcases in the current project");
    }
    ...
    public void reflesh()
    {
        ...
        autoUpdater.updateTestcase(new AutoUpdater.UpdateListener() {
            @Override
